window.PayslipModal = function PayslipModal({ isOpen, onClose, data }) {
    if (!isOpen || !data) return null;

    const PayslipPreview = window.PayslipPreview;
    const Payslip = window.Payslip;

    // Use A4 template (Payslip.jsx) when pre-calculated data is present
    const useA4 = data.useA4Template && data.calculatedEarnings && Payslip;

    const handleDownload = () => {
        const element = document.getElementById('payslip-a4-printable') || document.getElementById('payslip-printable-area');
        if (!element) {
            console.error('No printable payslip element found');
            return;
        }
        const opt = {
            margin: [5, 2, 5, 2],
            filename: `payslip_${data.name.replace(/ /g, '_')}_${(data.period || 'payslip').replace(/ /g, '_')}.pdf`,
            ...
            <div className="relative w-full max-w-4xl max-h-[90vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-20">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">{data.name} • {data.period || 'Current Period'}</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-1 zero-scrollbar bg-slate-50/50">
                    {useA4
                        ...
                        : <p className="p-8 text-center text-slate-500">No payslip data available.</p>
                    }
                </div>
            </div>
        </div>
